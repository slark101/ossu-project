...
# https://ocw.mit.edu/courses/6-0001-introduction-to-computer-science-and-programming-in-python-fall-2016/resources/ps3/
...
def get_word_score(word:str, n:int):
    ...
    first_score:int = 0
    for i in word:
        first_score+= SCRABBLE_LETTER_VALUES.get(i.lower(),0)
    return first_score*max(1,7*len(word)-3*(n-len(word)))
...
def update_hand(hand:dict[str,int], word:str):
    ...
    """
    new_hand = hand.copy()
    word_frequency = get_frequency_dict(word.lower())
    for i in word_frequency.keys():
        if(new_hand.get(i,0)):
            new_hand[i] -= word_frequency[i]
    return new_hand
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while hand_length:
        ...
        display_hand(hand)
        print("Please enter a word or '!!' to indicate you are done:",end=' ')
        word = input()
        if input == '!!':
            break        
...
def substitute_hand(hand:dict[str,int], letter:str):
    ...
    copy_hand = hand.copy()
    hand_letter = copy_hand.keys()
    if letter not in hand_letter:
        return hand
    for i in hand_letter:
        letters = letters.replace(i,'')
    new_letter = random.choice(letters)
    copy_hand[new_letter] = copy_hand.pop(letter)
    return copy_hand       
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    hand_number = int(input())
    score = 0
    hand = None
    allow_substitute = True
    allow_replay_hand = True
    while hand_number:
        proceed_replay = ''
        if allow_replay_hand and hand:
            print('Would you like to replay the hand?',end=' ')
            proceed_replay = input()
            if proceed_replay == 'yes':
                allow_replay_hand = False
        if proceed_replay!='yes':
            hand = deal_hand(HAND_SIZE)        
        if allow_substitute and proceed_replay != 'yes':
            print('Current hand:',end=' ')
            display_hand(hand)
            print('Would you like to substitute a letter?',end=' ')
            proceed_substitute = input()
            if proceed_substitute == 'yes':
